...
const DrawPrestigePage: React.FC = () => {
  ...
  const doneDrawing = async () => {
    ...
    const blocks = [...(session.blocks || [])]; // creates copy of blocks array
    ...
    }); // pushes new block to the local blocks array
  ...
  const getModalText = (): string => {
    const characterName = getCharacterForCondition('prestige');
